...
def build_admin_gestion_home(page: ft.Page) -> ft.Column:
    ...
    from database.queries import (
        ...
        obtenerPrestamosProximosVencerGeneral,
    )
    ...
    # ── 2.5) Notificaciones de vencimiento próximo (General) ─────
    proximos_vencer = obtenerPrestamosProximosVencerGeneral()

    def _fila_proximo_vencer(p):
        """
        p: (idPrestamo, nombreUsuario, ejemplar, fechaVencimiento, titulo)
        """
        return ft.Container(
            border=ft.Border.all(1, ft.Colors.GREY_200),
            border_radius=6,
            ...
            content=ft.Row(
                ...
                controls=[
                    ft.Column(
                        spacing=1, tight=True, expand=True,
                        controls=[
                            ft.Text(f"#{p[0]}  {p[4]}", size=11, weight=ft.FontWeight.W_600,
                                    color=ft.Colors.BLACK, max_lines=1,
                                    overflow=ft.TextOverflow.ELLIPSIS),
                            ft.Text(f"{p[1]}  •  Ej. #{p[2]}", size=9,
                                    color=ft.Colors.GREY_600, max_lines=1),
                        ],
                    ),
                    ft.Text(_fmt_fecha(p[3]), size=9, weight=ft.FontWeight.W_600, color=ft.Colors.AMBER_800),
                ],
            ),
        )

    proximos_vencer_controls = [_fila_proximo_vencer(p) for p in proximos_vencer] if proximos_vencer else [_empty_msg("Sin alertas")]
    ...
    def _on_change_desde(e):
        if e.control.value:
            tf_desde.value = e.control.value.strftime("%d/%m/%Y")
            _refrescar_libros()

    def _on_change_hasta(e):
        if e.control.value:
            tf_hasta.value = e.control.value.strftime("%d/%m/%Y")
            _refrescar_libros()

    picker_desde = ft.DatePicker(
        on_change=_on_change_desde,
        first_date=datetime.datetime(2025, 1, 1),
        last_date=datetime.datetime(2030, 12, 31),
    )
    picker_hasta = ft.DatePicker(
        on_change=_on_change_hasta,
        first_date=datetime.datetime(2025, 1, 1),
        last_date=datetime.datetime(2030, 12, 31),
    )

    page.overlay.append(picker_desde)
    page.overlay.append(picker_hasta)

    def _open_desde(e):
        picker_desde.open = True
        page.update()

    def _open_hasta(e):
        picker_hasta.open = True
        page.update()

    tf_desde.suffix = ft.IconButton(
        icon=ft.Icons.CALENDAR_MONTH,
        on_click=_open_desde,
        icon_size=18,
        padding=0,
    )
    tf_hasta.suffix = ft.IconButton(
        icon=ft.Icons.CALENDAR_MONTH,
        on_click=_open_hasta,
        icon_size=18,
        padding=0,
    )

    def _limpiar_fechas(e):
        tf_desde.value = ""
        tf_hasta.value = ""
        _refrescar_libros()
    ...
    return ft.Column(
        ...
        controls=[
            ...
            # Fila: Activos + Vencidos + Notificaciones (Vencen pronto)
            ft.Row(
                spacing=12,
                controls=[
                    ft.Container(
                        ...
                        content=ft.Column(
                            ...
                            controls=[
                                ft.Row(
                                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                                    controls=[
                                        ft.Text("Alertas (Vencen pronto)", size=14,
                                                weight=ft.FontWeight.W_600,
                                                color=ft.Colors.BLACK),
                                        ft.Container(
                                            padding=ft.Padding(6, 2, 6, 2),
                                            ...
                                            content=ft.Text(str(len(proximos_vencer)), size=11,
                                                            weight=ft.FontWeight.W_700,
                                                            color=ft.Colors.WHITE),
                                        ),
                                    ],
                                ...
                                ft.Container(
                                    ...
                                    content=ft.Column(
                                        ...
                                        scroll=ft.ScrollMode.AUTO,
                                        tight=True,
                                        controls=proximos_vencer_controls,
                                    ),
                                ),
                            ],
                        ),
