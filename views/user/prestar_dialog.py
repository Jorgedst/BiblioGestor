...
from database.queries import (
    ...
    obtenerEjemplaresNoDisponibles,
    registrarReserva,
)
...
def _open_error_dialog(page: ft.Page, mensaje: str, isbn: str = None):
    """Overlay de error para cuando no hay ejemplares disponibles, con opción de reservar."""
    ...
    def _hacer_reserva(e):
        ...
        # Obtener los ejemplares no disponibles
        ejemplares_no_disp = obtenerEjemplaresNoDisponibles(isbn)
        if not ejemplares_no_disp:
            _close_overlay()
            _open_error_dialog(page, "Este libro no tiene ejemplares físicos registrados para reservar.")
            return

        # Tomar el primer ejemplar no disponible para la reserva (ejemplares_no_disp[0] es la fila, [0][0] es el idEjemplar)
        id_ejemplar_reservar = ejemplares_no_disp[0][0]

        ok, res_msg = registrarReserva(codigo_usuario, id_ejemplar_reservar)
        if ok:
            _close_overlay()
            open_succesful_dialog(
                page,
                f"Reserva registrada con éxito para el ejemplar #{id_ejemplar_reservar}.\n"
                "Te notificaremos cuando esté disponible.",
            )
        else:
            _close_overlay()
            _open_error_dialog(page, f"Error al realizar la reserva: {res_msg}")

    # Preparar los botones de acción
    botones_row = ft.Row(
    ...
    if isbn:
        # Si se especificó el ISBN, verificar si hay ejemplares para reservar
        ejemplares_no_disp = obtenerEjemplaresNoDisponibles(isbn)
        if ejemplares_no_disp:
            botones_row.controls.append(
                ft.FilledButton(
                    content="Reservar",
                    width=120,
                    on_click=_hacer_reserva,
                    style=ft.ButtonStyle(
    ...
    modal_content = ft.Container(
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Text(
                    value="No disponible" if not isbn else "Sin ejemplares libres",
                    size=22,
    ...
    book_card = ft.Container(
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Text(
                    value=f"ISBN: {libro.get('isbn', 'No especificado')}",
                    size=12,
    ...
    def _realizar_prestamo(e):
        ...
        if id_ejemplar is None:
            ...
            _open_error_dialog(
                ...
                "¿Deseas realizar una reserva de este libro?",
                isbn=isbn
            )
            return
